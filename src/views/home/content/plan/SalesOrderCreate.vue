      ...
      <div class="table" id="listScroll"  v-show="showSearchView===false">
        <ul>
          <li v-for="item in list" :key="item.id" @click="showDetailView(item.id)">
            ...
            <div class="content">
              <span class="clientname">客户:{{"【"+item.client.name+"】"}}</span>
              <span class="clientcode">{{item.client.code}}</span>
              <span class="dataTime">交期:{{item.delivery_time}}</span>
    ...
    <div  class="detail"  v-show="showViewid==='detail'">
      ...
      <div class="content">
          ...
          <div >
            ...
            <span class="code">{{client.name}}</span>
          </div>
          <div >
            <span class="name">客户编码:</span>
        ...
        <div >
         ...
         <dl class="childItem">
              <li v-for="item in detail.child" :key="item.id">
                  <span class="typeName">产品类型:{{"【"+item.productType_name+"】"}}</span>
                  <span class="typeCode">{{item.productType_code}}</span>
                  <span class="name">产品:{{"【"+item.product_name+"】"}}</span>
                  <span class="code">{{item.product_code}}</span>
                  <span class="sum">需求量:{{item.sum}}</span>
                  <span class="assigned">分配量:{{item.assigned}}</span>
                  <span class="completed">完成数量:{{item.completed}}</span>
              </li>
         </dl>
        <dl>
          <dt>文件:</dt>
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('plan/salesOrderCreate/?ordering=' + self.selectItem.ordering).then(function (response) {
...
<style scoped lang="scss" >
  ...
  .detail .content .childItem  li .typeName{
    position: absolute;
    top:0;
    left: 0;
    width: 60%;
  ...
  .detail .content .childItem  li .typeCode{
    position: absolute;
    top:0;
    left: 60%;
    width: 40%;
  ...
  .detail .content .childItem  li .assigned{
    position: absolute;
    top:66%;
    left: 33%;
    width: 33%;
  ...
  .detail .content .childItem  li .completed{
    position: absolute;
    top:66%;
    left: 66%;
    width: 33%;
