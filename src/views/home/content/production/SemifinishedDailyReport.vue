<template>
  <div class="semifinishedDailyReport">
      ...
      <div class="content">
        ...
        <dl class="childItem">
          <li v-for="item in detail.child" :key="item.id">
            <span class="semifinishedType_name">产品类型:{{"【"+item.semifinishedType_name+"】"}}</span>
            <span class="semifinishedType_code">{{item.semifinishedType_code}}</span>
            <span class="semifinished_name">产品:{{"【"+item.semifinished_name+"】"}}</span>
            <span class="semifinished_code">{{item.semifinished_code}}</span>
            <span class="all_sum">总量:{{item.all_sum}}</span>
    ...
    <div  class="create"  v-show="showViewid==='create'">
      ...
      <form   ref="createForm" >
        <div>半成品生产日报名称:
          <input v-model="formItem.name"  placeholder="请输入半成品生产日报名称:...">
        </div>
        <div>半成品生产日报编码:
          <input v-model="formItem.code"  placeholder="请输入半成品生产日报编码:...">
        </div>
        <div>班组:
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('production/semifinishedDailyReport/?ordering=' + self.selectItem.ordering).then(function (response) {
  ...
  created () {
    ...
    this.$axios.get('user/userInfor/?page_size=99999&ordering=-id').then(function (response) {
      ...
      self.$axios.get('production/teamInfor/?page_size=99999&ordering=-id&state=使用中').then(function (response) {
...
<style scoped lang="scss" >
  ...
  .detail .content .childItem  li .semifinishedType_name{
    position: absolute;
    top:0;
    left: 0;
    width: 60%;
  ...
  .detail .content .childItem  li .semifinishedType_code{
    position: absolute;
    top:0;
    left: 60%;
    width: 40%;
  ...
  .detail .content .childItem  li .semifinished_name{
    position: absolute;
    top:33%;
    left: 0;
    width: 60%;
  ...
  .detail .content .childItem  li .semifinished_code{
    position: absolute;
    top:33%;
    left: 60%;
    width: 40%;
  ...
  .detail .content .childItem  li .all_sum{
    position: absolute;
    top:66%;
    left: 0;
    width: 33%;
