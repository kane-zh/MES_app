  ...
  <div class="productRouteInfor">
    ...
    <div  class="detail"  v-show="showViewid==='detail'">
      ...
      <div class="content">
        ...
        <div >
          <span class="name">关联工位:</span>
        </div>
        <dl class="childItem">
          <li v-for="item in detail.station" :key="item.id">
            <span class="name">工位:{{"【"+item.name+"】"}}</span>
            <span class="code">{{item.code}}</span>
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('process/productRoute/?ordering=' + self.selectItem.ordering).then(function (response) {
