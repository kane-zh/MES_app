  ...
  <div class="semifinishedStock">
    ...
    <div  class="list">
      ...
      <div class="search" v-show="showSearchView===true">
        ...
        <div>半成品类型：
          <select v-model="selectItem.semifinishedType">
            <option selected hidden disabled value="">请选择半成品类型</option>
            <option v-for="item in semifinishedType" :value="item.code" :key="item.code">{{item.name}}</option>
          </select>
        </div>
        <div>状态:
        ...
        <ul>
          <li v-for="item in list" :key="item.id" @click="showDetailView(item.id)">
            <div class="heard">
              ...
              <span class="state" v-show="item.state==='使用中'" style="background: #476ffe;">用</span>
            ...
            <div class="content">
              <span class="position_name">仓位:{{"【"+item.position_name+"】"}}</span>
              <span class="position_code">{{item.position_code}}</span>
              <span class="goodsType_name">半成品类型：{{"【"+item.semifinishedType_name+"】"}}</span>
              <span class="goodsType_code">{{item.semifinishedType_code}}</span>
              <span class="goods_name">半成品{{"【"+item.semifinished_name+"】"}}</span>
              <span class="goods_code">{{item.semifinished_code}}</span>
...
export default {
  ...
  methods: {
    ...
    select () {
      ...
      var self = this
      this.$axios.get('warehouse/semifinishedStockDetail/?state=' + self.selectItem.state +
    ...
    clear () {
      ...
      this.searchItem.semifinishedType = ''
...
<style scoped>
  ...
  .list .table .content .position_name{
    position: absolute;
    top:0;
    left: 1%;
    width: 60%;
  ...
  .list .table .content .position_code{
    position: absolute;
    top: 0;
    left: 60%;
    width: 40%;
