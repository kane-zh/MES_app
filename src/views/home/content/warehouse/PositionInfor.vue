        ...
        <ul>
          <li v-for="item in list" :key="item.id" @click="showDetailView(item.id)">
            <div class="heard">
              ...
              <span class="state" v-show="item.state==='闲置'" style="background-color: #61c255">闲</span>
              <span class="state" v-show="item.state==='使用中'" style="background-color: #5567c2">占</span>
...
<!--          <li>{{"仓库:状态:"+"&#12288;"+type.state}}</li>-->
          ...
          <div >
            ...
            <span class="code">{{detail.place}}</span>
          </div>
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('warehouse/position/?ordering=' + self.selectItem.ordering).then(function (response) {
  ...
  watch: {
    'type': function (newval, oldval) {
      ...
      this.$axios.get(`warehouse/warehouse/` + newval.id).then(function (response) {
